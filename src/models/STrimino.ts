...
export class STrimino extends SomeTrimino {
	uuid: String = uuid();
	meshes: THREE.Mesh[] = [];
	...
	constructor() {
		...
		cube1.position.set(c.GRID_SIZE, c.GRID_SIZE, 0);
		cube2.position.set(0, c.GRID_SIZE, 0);
		cube3.position.set(0, 0, 0);
		cube4.position.set(c.GRID_SIZE * -1, 0, 0);

		cube1.name = 'S1-' + this.uuid;
		cube2.name = 'S2-' + this.uuid;
		cube3.name = 'S3-' + this.uuid;
		cube4.name = 'S4-' + this.uuid;
